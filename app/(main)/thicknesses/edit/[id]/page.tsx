// app/(main)/thicknesses/edit/[id]/page.tsx
...
export default async function EditThicknessPage({ params }: EditThicknessPageProps) {
  ...
  // Fetch thickness data server-side using existing action
  ...
  return (
    <div className="flex min-h-screen w-full flex-col">
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
        ...
        <Card className="p-6">
          <FormClient thickness={thickness} returnUrl={`/thicknesses/${thicknessId}`} />
        </Card>
      </main>
    </div>
  )
